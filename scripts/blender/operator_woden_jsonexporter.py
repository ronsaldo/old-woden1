...
def convertCoordinate(vector):
    return Vector((vector.y, vector.z, vector.x))
...
def convertVertexWeights(groups, groupWeights):
    ...
    for i in range(min(len(groupWeights), 4)):
        indices[i] = groups[groupWeights[i].group].index
...
class WodenExporter:
    def __init__(self):
        ...
        self.boneDictionary = {}
        self.bones = [Bone(0, None, '_exporter_root', Vector((0.0, 0.0, 0.0)), Matrix())]
    ...
    def addBone(self, bbone):
        ...
        bone = Bone(-1, parent, bbone.name, bbone.tail, bbone.matrix)
    ...
    def addObject(self, object):
        ...
        for modifier in object.modifiers:
            if modifier.type == 'ARMATURE' and modifier.use_vertex_groups:
                ...
                self.addArmature(armature.data)
...
def export_woden_json(context, filepath, exportSelected, prettyPrint):
    ...
    if exportSelected:
        ...
        for obj in context.scee.objects:
            exporter.addObject(obj)
